...
public class AnnotationGenerator {
	...
	public void parseAnnotations(String digitalObjID, String annotationString) throws RepositoryException, RepositoryConfigException,
...
//		store to apache Jena
	 ...
	 */
	private void processXML(PcGtsType pcgtsTypeObj)
		...
		createPageAnnotation(pcgtsTypeObj, softAgentResourceID);
		createOtherAnnotations(pcgtsTypeObj,softAgentResourceID);
		System.out.println("done all processing");
	}

	private void createOtherAnnotations(PcGtsType pcgtsTypeObj, String softAgentResourceID) throws RepositoryException, IllegalAccessException, InstantiationException, MalformedQueryException, UpdateExecutionException {
		...
		for (RegionType regions : pcgtsTypeObj.getPage().getTextRegionOrImageRegionOrLineDrawingRegion()) {
			...
			createOtherBodyTarget(annoations, regions);
		}
		System.out.println("all annotations are created");
		
	}

	private void createOtherBodyTarget(Annotation annoations, RegionType regions) throws RepositoryException, IllegalAccessException, InstantiationException {
//		useless code but it is requried as it is not printing other discriptions without  this initialisations.
		...
		SpecificResource specific = getTarget(regions.getCoords().getPoints());
	...
	private void createPageAnnotation(PcGtsType pcgtsTypeObj, String softAgentResourceID)
			throws RepositoryException, IllegalAccessException, InstantiationException, MalformedQueryException, UpdateExecutionException {
		Annotation annoations = createAnnoataionPart(pcgtsTypeObj.getMetadata(), softAgentResourceID);
		createBodyTarget(annoations, pcgtsTypeObj.getPage());

	}

	private void createBodyTarget(Annotation annoations, PageType page) throws RepositoryException, IllegalAccessException, InstantiationException {
		...
		SpecificResource specific = getTarget(page.getBorder().getCoords().getPoints());
	...
	private SpecificResource getTarget(String points) throws RepositoryException, IllegalAccessException, InstantiationException {
	 ...
	 * @throws RepositoryException
	 */
	private String checkAgentInRegistry(MetadataType metadata) throws RepositoryException, IllegalAccessException, InstantiationException {

		QueryExecution query = QueryExecutionFactory.sparqlService(ServiceURI, qureyUtil.checkAgentQuery(metadata.getCreator()));
